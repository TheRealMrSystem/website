...
// Step 2
  ...
  $('#step2next').on('click', function() {
    ...
    var linemanageremailValid = ($('#linemanageremail-input').val().length !== 0);
    ...
    $('#linemanageremail-alert').toggle(!linemanagerValid);
    ...
    // If all required fields are valid, go to next step
    if (jobtitleValid && departmentValid && linemanagerValid && linemanageremailValid && jobdescriptionValid && seniorityValid && timewithcompanyValid) {
      // Scroll to the top of the page
      $(document).scrollTop(0);
      // "tap" the relevant slider nav to skip to next slide
